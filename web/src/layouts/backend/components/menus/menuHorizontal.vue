                        ...
                        <el-sub-menu :index="menu.path" :key="menu.path">
                            <template #title>
                                <Icon
                                    ...
                                    :name="menu.meta?.icon ? menu.meta?.icon as any : config.layout.menuDefaultIcon"
                                />
                                <span>{{ menu.meta?.title ? menu.meta?.title as any : '无标题' }}</span>
                            </template>
                    ...
                    <template v-else>
                        <el-menu-item v-blur :index="menu.path" :key="menu.path" @click="onClickMenu(menu)">
                            <Icon
                                ...
                                :name="menu.meta?.icon ? menu.meta?.icon as any : config.layout.menuDefaultIcon"
                            />
                            <span>{{ menu.meta?.title ? menu.meta?.title : '无标题' }}</span>
                        </el-menu-item>
                    </template>
                </template>
            </el-menu>
